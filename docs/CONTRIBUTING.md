...
```
DNA-utils-universal/
├── str-matcher/          # Frontend (Next.js 14)
├── backend/              # Backend Service (Node.js/Express)
├── ftdna_haplo/          # Legacy Haplo Service (Node.js)
└── docs/                 # Документация
```
...
- Логика поиска в `workers/`
- Компоненты в `components/str-matcher/`

#### Backend Service
- API маршруты в `routes/`
- Работа с БД (PostgreSQL) через `pg-promise` или ORM
- Очереди задач через Redis/BullMQ

#### FTDNA Haplo Service
- API для деревьев гаплогрупп
...
2. **Проверьте работу API**:
   - Основные endpoints (Search, Haplo)
   - Обработка ошибок
   - CORS настройки (порт 9002/3000 -> 9004)
...
Примеры:
```
feat: добавить поддержку GIST индексов для PostgreSQL
fix: исправить capping генетической дистанции
docs: обновить диаграмму архитектуры v2
```
...
3. **Производительность**: Отсутствие блокировок UI (Worker-first)
4. **Безопасность**: Валидация входных данных, CORS
...
1. **Оптимизация производительности** (SQL/Workers)
2. **Улучшение UX/UI** (Accessibility)
